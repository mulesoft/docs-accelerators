...
Please refer to the xref:./fins-salesforce-cdp-setup-guide.adoc[Salesforce CDP setup guide] for instructions on how to publish and activate a CDP segment. Once the segment is published and activated, it will trigger the creation of a data extension in Marketing Cloud with the same name as the CDP segment.
