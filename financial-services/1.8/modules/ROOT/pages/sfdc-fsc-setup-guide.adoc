...
* The Apex Triggers AccountEventsPub, AccountAddressEventsPub, and HouseholdEventsPub filter records based on record type ID. A query is provided in each Apex trigger as a comment and can be used to obtain the record type ID. 
