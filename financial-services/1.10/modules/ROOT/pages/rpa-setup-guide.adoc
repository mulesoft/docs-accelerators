...
.. The first project contains the process for getting and looping over all relevant data sets in the AMS. For each data set, it calls the second project.
.. The second project contains the process for getting the data from one AMS data set and adding it to a CSV file, which later is imported to FSC.
