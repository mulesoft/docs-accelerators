...
. Export the data, either to a CSV file or via a REST call.

image::rpa-setup-export-data.png[RPA Builder export data]

Change the following Action Steps:

. To retrieve the data, re-identify the fields from which to read in all the Read Text from App Element Action Steps and add or delete them as needed.
...
. Ensure all retrieved data is pinned to the Combine Strings Action Steps and adjust if necessary. You can also link more Activity Parameters. 
+
image::rpa-setup-combine-strings-wizard.png[RPA Builder Combine Strings Wizard]

. Add the string to a CSV file via the `Pin Variable`. You can adjust the properties as needed.
+
image::rpa-setup-write-text-file-wizard.png[RPA Builder Write to Text File Wizard]

The CSV file must be in the format specified.
