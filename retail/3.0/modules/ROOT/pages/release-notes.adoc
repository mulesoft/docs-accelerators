= MuleSoft Accelerator for Retail Release Notes
:retail-version: 3.0
...
== Updates to Existing Customer Sync Use Case

This release includes the updates to Customer sync use case, which synchronizes and unifies customer creation and updates across relevant applications, with one application serving as the source of the customer. This release also includes a simplified solution with Salesforce Core as the global data store. It also includes configuration based solution to include System APIs of choice in the Accelerator Customer Process API.
...
* xref:retail::index.adoc[MuleSoft Accelerator for Retail]
