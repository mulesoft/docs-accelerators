...
Your Salesforce org is now connected. With the connection is established, the Salesforce CDP admin can either use bundles that can automatically deploy data or set up their own data streams.
...
Cleaning and preparing your data is critical for success in using Salesforce CDP's segmentation and activation capabilities.
...
* https://help.salesforce.com/s/articleView?id=sf.c360_a_data_mapping_views.htm&type=5[Data Mapper Views] - Select table view or visual view when mapping your data in Salesforce CDP.
