...
* The Apex Trigger that is created filters records based on `LastModifiedBy <> {Service account ID}` to avoid cyclical updates performed by the Salesforce Customers System API. Run the query `SELECT Id FROM User WHERE Username='<service-account-username>'` in the Developer Console to obtain the Apex Trigger.
