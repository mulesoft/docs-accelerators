...
Use the`CustomObjectUtility` and `MetadataService` Apex classes to create a custom object programmatically. The source is on the https://anypoint.mulesoft.com/exchange/org.mule.examples/rcg-common-resources-src/[RCG Common Resources] project and https://github.com/financialforcedev/apex-mdapi/blob/master/apex-mdapi/src/classes/MetadataService.cls[MetadataService] respectively. Download the project and follow these steps:
...
[%header%autowidth.spread]
|===
| Salesforce Object Label | Field Label | Field Name | Data Type | Length | Unique | ExternalId a| Formula
...
| -
| -
| Shipping＿c + Total_Tax＿c + TotalAmount
...
Use the Apex class `CustomFieldUtility` to create custom fields programmatically. See the https://anypoint.mulesoft.com/exchange/org.mule.examples/rcg-common-resources-src/[RCG Common Resources] project to find the source for this class. Once you have downloaded the project, follow these steps:
...
1. All `Global_*_Id__c` fields should be visible in layouts, as described previously, but made *read-only* to avoid issues with data synchronization.
====
...
| Managed External Identifier | Status | Status__c | Picklist | VALID, INVALID
| Product | Product Group | Product_Catalog__c | Picklist| Basic, Premium
| Product | Status | Status__c | Picklist| VALID, INVALID
|===
...
. Click *Save*.
. Enable the OAuth 2.0 Username-Password Flow by following the steps documented in https://help.salesforce.com/s/articleView?id=release-notes.rn_security_username-password_flow_blocked_by_default.htm&release=244&type=5[Salesforce Help Docs].
...
Download the https://anypoint.mulesoft.com/exchange/org.mule.examples/rcg-common-resources-src[RCG Common Resources project^] for a sample `settings.xml` file that can be used as a starting point.
...
The following Platform Event Object and Apex Triggers must be created in order to capture updates from Salesforce. The scripts to create these Apex Triggers and add fields to Platform Event Object are located in the https://anypoint.mulesoft.com/exchange/org.mule.examples/rcg-common-resources-src/[RCG Common Resources] project.
...
. To add Custom Fields & RelationShips to Platform Event Definitions, run the utility script `CreatePlatformEventsCustomFields.txt` available in the `rcg-common-resources/salesforce` folder of https://anypoint.mulesoft.com/exchange/org.mule.examples/rcg-common-resources-src/[RCG Common Resources] project.
...
. Click *New*.
. Search for the Apex Trigger name, which is indicated in the previous table. Copy the Trigger Script code `CreateApexTriggers.txt` available in the `rcg-common-resources/salesforce` folder of https://anypoint.mulesoft.com/exchange/org.mule.examples/rcg-common-resources-src/[RCG Common Resources] project and paste it into the script editor.
. Obtain the `LastModifiedById` by running the query from the Additional Notes section.
